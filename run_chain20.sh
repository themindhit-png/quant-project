#!/bin/bash
# chain 20: new-alpha research (exp24a), slow ML (exp24b), age-free 8h model (exp24c), then the Opus-round-2 harness block
cd /agent/workspace/proj
while pgrep -f "exp23a_f_check[s]" > /dev/null; do sleep 15; done
echo "=== $(date -u +%H:%M:%S) exp24a"; python3 -u exp24a_new_alpha.py > logs/exp24a.log 2>&1
echo "=== $(date -u +%H:%M:%S) exp24b"; python3 -u exp24b_slow_ml.py > logs/exp24b.log 2>&1
echo "=== $(date -u +%H:%M:%S) exp24c"; python3 -u exp24c_ml8_noage.py > logs/exp24c.log 2>&1
