#!/usr/bin/env python3
"""Experiment 19c: stability grid of the 5-sleeve book on the 8h engine around the chosen config
(smooth 0.5 per 8h step, band 0.3, ml8 weight 2): smooth {0.4,0.5,0.6} x weight {1.5,2,2.5} + band {0.2,0.4}."""
...
def combo(parts, weights):
    ...
    return fn
def book(w8): return combo([Held(sl_listing), Held(sl_core), Held(make_ml(P3)), Held(make_ml(P7)), make_ml(P8, 0.3)], [1, 1, 1, 1, w8])
rows = []
def go(label, fn, **kw):
    ...
    rows.append(dict(label=label, sharpe=m['sharpe'], cagr=m['cagr'], mdd=m['mdd'], turnover=m['turnover_x']))
    print(f'{label:<40} Sh {m["sharpe"]:5.2f} CAGR {m["cagr"]*100:6.1f}% dvol {m["dvol"]*100:.2f}% MDD {m["mdd"]*100:6.1f}% wd {m["worst_day"]*100:5.1f}% turn {m["turnover_x"]:4.0f}x cost {(m["fees"]+m["slip"])*100:.1f}% | '
          + ' '.join(f'{y}:{v[1]:.1f}' for y, v in by.items()) + f' ({time.time()-t1:.0f}s)', flush=True)
for sm in (0.4, 0.5, 0.6):
    for w8 in (1.5, 2.0, 2.5):
        go(f'smooth{sm} w8={w8}', book(w8), smooth=sm)
go('smooth0.5 w8=2 band0.2', book(2.0), band=0.2)
go('smooth0.5 w8=2 band0.4', book(2.0), band=0.4)
go('smooth0.5 w8=2 VT0.5', book(2.0), vol_target=0.005)
df = pd.DataFrame(rows); print(f'\nSharpe over grid: mean {df.sharpe.mean():.2f} min {df.sharpe.min():.2f} max {df.sharpe.max():.2f} std {df.sharpe.std():.2f}')
df.to_csv('out_exp19c_stability.csv', index=False); print('DONE')
